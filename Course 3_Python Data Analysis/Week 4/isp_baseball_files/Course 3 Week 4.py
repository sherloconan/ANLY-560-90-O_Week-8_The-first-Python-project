...
def filter_by_year(statistics, year, yearid):
    ...
    for row in statistics:
        #Input year as integer while values of yearid are string
        if row.get(yearid) == str(year):
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        ...
        #adding a tuple of playerid and stat into the list
        full_list.append((row[info["playerid"]], stat))
    full_list.sort(key=lambda pair:pair[1], reverse=True)
    #Function assumes no ties
    top_list = [full_list[idx] for idx in range(numplayers)]
    return top_list
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    for row in top_ids_and_stats:
        #Or is better to use .get()
        name = masterfile[row[0]][info["firstname"]] + " " + masterfile[row[0]][info["lastname"]]
        #below is a string
        stat = "{0:.3f}".format(row[1])
        stat_and_name.append(stat + " --- " + name)
    return stat_and_name
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    my_statistics = read_csv_as_list_dict(info["battingfile"],
                                          info["separator"],
                                          info["quote"])
    list_by_year = filter_by_year(my_statistics, year, info["yearid"])
    top_list = top_player_ids(info, list_by_year, formula, numplayers)
    result_list = lookup_player_names(info, top_list)
    return result_list
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    newdict = {}
    for row in statistics:
        if row[playerid] not in newdict:
            #statistics were stored as string
            newdict[row[playerid]] = {col:int(row[col]) for col in fields}
        else:
            for col in fields:
                newdict[row[playerid]][col] += int(row[col])
    for my_key in newdict:
        newdict[my_key][playerid] = my_key
    return newdict
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    my_statistics = read_csv_as_list_dict(info["battingfile"],
                                          info["separator"],
                                          info["quote"])
    aggregated_statistics_dict = aggregate_by_player_id(my_statistics,
                                                        info["playerid"],
                                                        info["battingfields"])
    aggregated_statistics_list = []
    for my_key in aggregated_statistics_dict:
        aggregated_statistics_list.append(aggregated_statistics_dict[my_key])
    top_list = top_player_ids(info, aggregated_statistics_list, formula, numplayers)
    result_list = lookup_player_names(info, top_list)
